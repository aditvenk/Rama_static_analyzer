...
convertGEP (ConstantExpr * CE, Instruction * InsertPt) {
  ...
  //
  if(GetElementPtrInst *gepInst = dyn_cast<GetElementPtrInst>(CE->getAsInstruction())){
	gepInst->insertBefore(InsertPt);
	return gepInst;
  }
  /*
  return (GetElementPtrInst::Create (CE->getOperand(1)->getType(),
                                     CE->getOperand(1),
                                     Indices,
                                     CE->getName(),
                                     InsertPt));
